...
import { Radio, X, Send, Globe, Users, Anchor, RefreshCw } from 'lucide-react';
...
export default function DistressSignalsFeed({ onClose, currentUserId, ownedIslandIds, friends }: DistressSignalsFeedProps) {
  ...
  const tabs: { id: FeedTab; label: string; icon: typeof Globe }[] = [
    ...
    { id: 'my-islands', label: 'My Islands', icon: Anchor },
  ];
    ...
    <div className="flex flex-col h-full max-h-[85vh]">
      ...
      <div className="flex items-center justify-between mb-5 shrink-0">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => fetchDistressFeed(currentUserId)}
            disabled={loading}
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pr-1 min-h-0">
        ...
        {!loading && filteredFlares.length === 0 && (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {friends.includes(flare.askerId) && (
                      <span className="text-[9px] font-bold uppercase tracking-widest text-blue-400/70 bg-blue-400/10 px-1.5 py-0.5 rounded-md">
                        Crew
                      </span>
                    )}
                    {ownedIslandIds.includes(flare.islandId) && (
                      <span className="text-[9px] font-bold uppercase tracking-widest text-emerald-400/70 bg-emerald-400/10 px-1.5 py-0.5 rounded-md">
                        Your Island
                      </span>
                    )}
                  </div>
